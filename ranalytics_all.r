...
# data preparation
appData <- na.omit(featData)
appData <- scale(featData) # standardize variables
attach(featData)
head(featData)
summary(featData)
...
d <- dist(featData, method = "euclidean") # distance matrix
fit <- hclust(d, method="ward", rotate=TRUE) 
...
library(fpc)
plotcluster(featData, fit$cluster)
...
# kmeans
kclus <- kmeans(featData, centers= 7, iter.max=1000, nstart=10000)
...
ordiplot(cmd, type = "n")
cols <- c("steelblue", "darkred", "darkgreen", "pink", "blue", "lightgreen", "#bada55")
...
km <- kmeans(featData,10)
dissE <- daisy(featData) 
dE2 <- dissE^2
