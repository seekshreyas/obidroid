...
CheckApps
=========
Given an input of fair apps, extract the reveiws and create a csv file
"""
...
def getDataframeFromDatabase(host, db, un, pw):
    query = "select id, app, review from android_dataset.reviews as adr \
        join obidroid.fair_apps as ofa on adr.app = ofa.pkgname where ASCII(adr.review) != 0;"
    conn = MySQLdb.connect(host = host, user = un, passwd = pw, db = db)
    fair_app_reviews_df = psql.frame_query(query, conn)
    return fair_app_reviews_df
